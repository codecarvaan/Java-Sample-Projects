...
public class EventDAO {
	...
	public int insertEvent(Event insertEvent) throws ClassNotFoundException,
		...
		for (int i = 1; i <= insertEvent.getEventSession(); i++) {
			...
			resultSet = statement.executeQuery();
			resultSet.next();
				int eventSessionId = resultSet.getInt(1);
			...
			log.info("Details from insertEvent are : " + eventSessionId + " " + insertEvent.getEventCoordinatorId() + " " +eventId + " " +insertEvent.getSeatsavailable()  );
			//log.info("After " + insertEvent.display());
			status = statement.executeUpdate();
		}
		//log.info(insertEvent.display());
		resultSet.close();
		return status;
	}	
